...
class ControllerTests {
    ...
    public void givenId_CreditAndThenDebitMoreGetException_thenReturnJson() {
        ...
        ResponseEntity<TransactionStatus> response = controller.debit("17892", new WithdrawalTransaction(5000.0));
        ...
        if (response.getStatusCode() == HttpStatus.NOT_FOUND) {
            assertEquals(HttpStatus.NOT_FOUND, response.getStatusCode(), "Expected 404 Not Found but got: " + response.getStatusCode());

        } else {
            assertTrue(response.getStatusCode() == HttpStatus.OK, "Expected successful transaction but got: " + response.getStatusCode());
        }
    }
